...
async def get_asset_by_id(db_path: str, asset_id: int) -> AiAsset | None:
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        cursor = await db.execute("SELECT * FROM ai_assets WHERE id = ?", (asset_id,))
...
async def update_asset_fields(
    ...
    return await get_asset_by_id(db_path, asset_id)
...
async def get_usage_by_tag(
    ...
    for row in rows:
        ...
        totals[tag_val]["total_cost_usd"] += row["cost_usd"]
        totals[tag_val]["total_input_tokens"] += row["input_tokens"]
        totals[tag_val]["total_output_tokens"] += row["output_tokens"]
