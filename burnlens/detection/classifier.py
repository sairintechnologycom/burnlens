...
async def match_provider(endpoint_url: str, db_path: str) -> str | None:
    ...
    for sig in signatures:
        if fnmatch.fnmatch(url_lower, sig.endpoint_pattern):
            return sig.provider
...
async def upsert_asset_from_detection(
    ...
    if match is None:
        ...
        event = DiscoveryEvent(
            ...
            asset_id=asset_id,
            details={"provider": provider, "model": model_name, "source": "detection"},
        )
        ...
        await _update_last_active(db_path, match.id)
        logger.debug(
...
async def classify_new_assets(db_path: str) -> int:
    ...
    for asset in shadow_assets:
        ...
        if provider is None:
            logger.warning("Asset id=%s has unknown provider endpoint: %s", asset.id, asset.endpoint_url)
        count += 1
